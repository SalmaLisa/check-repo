...
import { saveERO, getERO, getEROByUser } from './../../services/eros';
...
class ERO extends Form {
	...
	async populateUser() {
		try {
			...
			const { data: user } = await getERO(userId);
			const ero = user[0];
			if (ero.dateBirth) ero.dateBirth = new Date();

			ero.firstName = ero.contactName.first;
			ero.lastName = ero.contactName.last;
			ero.initials = ero.contactName.initials;
